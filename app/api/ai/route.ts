...
//   /**
//    * Handles incoming POST requests to the /api/ai route.
//    *
//    * The request body should contain a JSON object with two properties:
//    * - `messages`: an array of messages to send to the AI, where each message is an object with `role` and `content` properties.
//    * - `chatId`: the ID of the chat to update, or `null` if a new chat should be created.
//    *
//    * The response will contain a JSON object with the following properties:
//    * - `generatedText`: the text generated by the AI in response to the input messages.
//    * - `chatId`: the ID of the chat that was updated or created.
//    * - `title`: the title of the chat, or `null` if no title was set.
//    */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session?.user?.email) {
      if (!chatId) {
        const titleResult = await model.generateContent(
          `Generate a short, catchy title (max 3 words) for a Christian conversation that starts with this message: "${
            messages[messages.length - 1].content
          }". Out of your many suggestions. Just stick with the first idea you come up with`
        );
        title = titleResult.response.text().replace(/^"(.*)"$/, "$1");
